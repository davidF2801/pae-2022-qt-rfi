% For K-band receiver
% Plots the ROC from the DC=0% PDF together with the one coming from ku_mean, sk_mean, ku and sk, generated from "K_simulation_repetitions".
% Saves the plots
...
ku_noise_mean = 3.0005;
ku_noise_std = 0.10938;

ku_ref_mean = ku_mean;
ku_ref_std = std(ku);

sk_noise_mean = -0.0094476;
sk_noise_std = 0.05485;
...
if(ku_mean<ku_noise_mean) %lower kurtosis threshold
...
saveas(fig1,strcat('K_ku_ROC_DC',num2str(DC(i)*100),'_INR',num2str(abs(INR(i))),'.fig'));
saveas(fig1,strcat('K_ku_ROC_DC',num2str(DC(i)*100),'_INR',num2str(abs(INR(i))),'.png'));
...
if(sk_mean<sk_noise_mean) %lower skewness threshold
...
xlabel('Probability of false alarm (P_f_a)');
ylabel('Probability of detection (P_d)');
saveas(fig2,strcat('K_sk_ROC_DC',num2str(DC(i)*100),'_INR',num2str(abs(INR(i))),'.fig'));
saveas(fig2,strcat('K_sk_ROC_DC',num2str(DC(i)*100),'_INR',num2str(abs(INR(i))),'.png'));
