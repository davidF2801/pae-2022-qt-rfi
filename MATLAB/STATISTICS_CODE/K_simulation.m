% Runs the whole K-band receiver pipeline. If desired,
%   - Plots the each step
%   - Saves each plot
...
%Cut-off frequencies of each filter
fpass11 = 20e6;
fpass12 = 40e6;                
fpass2 = 6e6;
fpass3 = 1600;
...
M = N/50;
offset = rand()*M;
i1 = A1*cos(2*pi*(f0/fs)*n + theta).*pulsetrain(n,M,DC,offset);
...
%% Figure 2: bandpass(signal + noise)

if(filter==1)
    y = bandpass(x, [fpass11 fpass12], fs);
elseif(filter==2)
    y = filtfilt(d1, x);
end
...
if(display)
    ...
    fig = figure('Name', 'bandpass(signal + noise)', 'Position', [10 250 1500 500]);
    ...
    sgtitle("bandpass(signal + noise)")
    if(save)
        saveas(fig, 'bandpass(signal+noise)', 'png');
    end
end
%% Figure 3: (bandpass(signal + noise))^2
...
if(display)
    ...
    fig = figure('Name', '(bandpass(signal + noise))^2', 'Position', [10 250 1500 500]);
    ...
    sgtitle("(bandpass(signal + noise))^2")
    if(save)
        saveas(fig, '(bandpass(signal+noise))^2', 'png');
    end
end
...
if(display)
    ...
    fig = figure('Name', '10*log10((bandpass(signal + noise))^2)', 'Position', [10 250 1500 500]);
    ...
    sgtitle("10*log10((bandpass(signal + noise))^2)")
    if(save)
        saveas(fig, '10log10((bandpass(signal + noise))^2)', 'png');
    end
end
%% Figure 5: lowpass(10*log10((bandpass(signal + noise))^2))
...
if(display && LP2applied)
    ...
    fig = figure('Name', 'lowpass(10*log10((bandpass(signal + noise))^2))', 'Position', [10 250 1500 500]);
    ...
    sgtitle("lowpass(10*log10((bandpass(signal + noise))^2))")
    if(save)
        saveas(fig, 'lowpass(10log10((bandpass(signal + noise))^2))', 'png');
    end
end

%% Figure 6: lowpass(lowpass(10*log10((bandpass(signal + noise))^2)))
...
if(display)
    ...
    fig = figure('Name', 'lowpass(lowpass(10*log10((bandpass(signal + noise))^2))', 'Position', [10 250 1500 500]);
    ...
    sgtitle("lowpass(lowpass(10*log10((bandpass(signal + noise))^2)))")
    if(save)
        saveas(fig, 'lowpass(lowpass(10log10((bandpass(signal + noise))^2))', 'png');
    end
end

%% Figure 7: sampling(lowpass(lowpass(10*log10((bandpass(signal + noise))^2))))
...
if(display)
    Nplot = 200;
    %Nplot = N_adc;
    fig = figure('Name', 'sampling(lowpass(lowpass(10*log10((bandpass(signal + noise))^2)))', 'Position', [10 250 1500 500]);
    ...
    sgtitle("sampling(lowpass(lowpass(10*log10((bandpass(signal + noise))^2)))")
    if(save)
        saveas(fig, 'sampling(lowpass(lowpass(10log10((bandpass(signal + noise))^2)))', 'png');
    end
end
...
title("Histogram of sampling(lowpass(lowpass(10*log10((bandpass(signal + noise))^2)))")
grid on
grid minor
if(save)
    saveas(fig, 'hist_sampling(lowpass(lowpass(10log10((bandpass(signal + noise))^2)))', 'png');
end